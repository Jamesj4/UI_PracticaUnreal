...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnAmmoChangedSignature,int, MaxAmountCount);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnDamageRecieve);
...
public:
	...
	void RecreateAbilityTree();

	UFUNCTION()
	void DisableSplashScreen();

	void ToogleAbilityTree();
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = AbilityTree, meta = (AllowPrivateAccess = "true"))
	TArray<FAbilityBranchInfo> AbilityBranches;
	...
	/** Look Input Action */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Input, meta = (AllowPrivateAccess = "true"))
	class UInputAction* ToogleAbilityTreeAction;
	...
	int32& GetNumberOfCoins(){return NumberOfCoins;}
	void SetNumberOfCoins(int32 n){NumberOfCoins = n;}
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = UI)
	TSubclassOf<USplashWidget> SplashHUDWidget;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = UI)
	TSubclassOf<UAbilityTreeHUD> AbilityTreeHUD;
	...
	FOnAmmoChangedSignature OnAmmoCountChanged;

	UPROPERTY(BlueprintAssignable, Category = "Health")
	FOnDamageRecieve OnDamageReceive;
...
private:
	...
	USplashWidget* SplashHUDInstance;
	UAbilityTreeHUD* AbilityHudInstance;


	bool bIsTreeOpen = false;
};
