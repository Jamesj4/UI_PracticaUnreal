...
void AUTAD_UI_FPS_Enemy::BeginPlay()
{
	...
	EnemyHealthBarWidgetComponent= FindComponentByClass<UWidgetComponent>();
	if(EnemyHealthBarWidgetComponent)
	{
		EnemyHealthBarWidget = Cast<UEnemyHealthBar>(EnemyHealthBarWidgetComponent->GetWidget());
		if(EnemyHealthBarWidget) EnemyHealthBarWidget->Show_Implementation();
	}
...
void AUTAD_UI_FPS_Enemy::SetHealth(int NewHealth)
{
	...
	{
		EnemyHealthBarWidget->UpdateEnemyHealthBar(Health,MaxHealth);
	}
}
